...
class Hashable(ABC):
    ...
    def get_previous_game_hash(self, hash_code: str) -> str:
        sha = hashlib.sha256()
        sha.update(hash_code.encode('utf-8'))
        return sha.hexdigest()

    def get_previous_n_games(self, hash_code: str, number_of_games: int = 10) -> List:
        results = []

        for count in range(number_of_games):
            results.append(self.get_result_by_hash(hash_code))
            hash_code = self.get_previous_game_hash(hash_code)

        return results

    def get_previous_games_to_hash(self, hash_code: str, hash_match: str = None) -> List:
